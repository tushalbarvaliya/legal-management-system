...
import { getLawyerById, type lawyerData, type userData } from "@/api/lawyerAPI";
import { queryClient } from "@/main";
import { useMutation, useQuery } from "@tanstack/react-query";
import { useForm } from "react-hook-form";
import { toast } from "sonner";
import type { AddLawyerFormData } from "./AddLawyerModel";
...
const UpdateLawyerModel = (data: AddModalProps) => {
  const { data: lawyerData } = useQuery<lawyerData>({
    queryKey: ["lawyer", data.id],
    queryFn: () => getLawyerById(data.id),
  });
  const {
    register,
    handleSubmit,
  ...
  useEffect(() => {
    if (lawyerData) {
      reset({
        ...
        userId: String(lawyerData.userId),
      });
    }
  }, [lawyerData, reset]);
