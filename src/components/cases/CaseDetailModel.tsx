      ...
      <DialogContent className="max-w-xl p-5 sm:p-6 lg:min-w-200">
        ...
        <DialogHeader className="flex flex-row items-start justify-between">
          <div>
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              Created At {formatDate(data.case.createdAt)}
            </p>
          </div>
        </DialogHeader>
        ...
        <div className="space-y-4 text-sm">
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <InfoCard label="Case Type" value={data.case.type} />

            <Link to={`/client/${data.case.clientId}`}>
              <InfoCard label="Client " value={`${data.user.firstName} ${data.user.lastName}`} />
            </Link>

            <InfoCard label="Case City" value={data.case.caseCity} />
            <InfoCard label="Case Stage" value={data.case.caseStage} />

            <InfoCard
              label="Expected Closing Date"
              value={formatDate(data.case.caseClosedDate)}
            />

            <InfoCard label="Case ID" value={String(data.case.id)} />
          </div>
        </div>
